...
import { getDailyReflectionById, updateDailyReflectionById } from 'apiSdk/daily-reflections';
...
function DailyReflectionEditPage() {
  ...
  const handleSubmit = async (values: DailyReflectionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDailyReflectionById(id, values);
      mutate(updated);
      resetForm();
