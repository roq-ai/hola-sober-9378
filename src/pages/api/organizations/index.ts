...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.daily_reflection?.length > 0) {
      const create_daily_reflection = body.daily_reflection;
      body.daily_reflection = {
        create: create_daily_reflection,
      };
    } else {
      delete body.daily_reflection;
    }
    if (body?.sober_meeting?.length > 0) {
      const create_sober_meeting = body.sober_meeting;
      body.sober_meeting = {
        create: create_sober_meeting,
      };
    } else {
      delete body.sober_meeting;
    }
